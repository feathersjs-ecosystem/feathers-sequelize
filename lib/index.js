...
class Service extends AdapterService {
  ...
  _get (id, params = {}) {
    const { query: where, filters } = this.filterQuery(params);
    ...
    if (filters.$select) {
      q.attributes = [...filters.$select, this.id].filter(
        (attr, index, self) => self.indexOf(attr) === index
      );
    }
  ...
  _patch (id, data, params = {}) {
    ...
    const handleReturn = (id, params, findParams) => {
      return () => {
    ...
    return ids.then(idList => {
      ...
      if (params.sequelize && params.sequelize.include) {
        const includeParams = Object.assign(
          {},
          params,
          { sequelize: Object.assign({}, params.sequelize, { raw: false }) }
        );
        return this._getOrFind(id, includeParams)
          .then(result => {
            if (Array.isArray(result)) {
              return Promise.all(result.map(instance => {
                return instance.update(_.omit(data, this.id))
              })).then(handleReturn(id, params, findParams))
            }
            return result.update(_.omit(data, this.id))
              .then(handleReturn(id, params, findParams));
          });
      }
  ...
  _update (id, data, params = {}) {
    ...
    if (filters.$select) {
      seqOptions.attributes = [...filters.$select, this.id].filter(
        (attr, index, self) => self.indexOf(attr) === index
      );
    }
  ...
  _remove (id, params = {}) {
    const getOrFindParams = Object.assign(
      {},
      params,
      { sequelize: Object.assign({}, { raw: this.raw }, params.sequelize) }
    );
    const { query: where, filters } = this.filterQuery(params);
    ...
    const seqOptions = Object.assign({}, { where }, params.sequelize);
    ...
    if (params.$returning !== false) {
      return this._getOrFind(id, getOrFindParams).then(data => {
        return Model.destroy(seqOptions).then(() => data);
      })
        .catch(utils.errorHandler);
    } else {
      return Model.destroy(seqOptions).then(() => Promise.resolve([]))
        .catch(utils.errorHandler);
    }
  }
}
