...
class Service extends AdapterService {
  ...
  _update (id, data, params = {}) {
    ...
    return this._get(id, { sequelize: seqOptions, query: where }).then(async instance => {
      const copy = Object.keys(instance.toJSON()).reduce((result, key) => {
        result[key] = typeof data[key] === 'undefined' ? instance[key] : data[key];

        return result;
      }, {});

      const update = await instance.update(copy, seqOptions)
        .then(() => this._get(id, {
