import { GeneralError, NotFound, Unavailable, Forbidden, Timeout, BadRequest, MethodNotAllowed } from '@feathersjs/errors';
import { _ } from '@feathersjs/commons';
import { AdapterBase, filterQuery, select } from '@feathersjs/adapter-commons';
...
class SequelizeAdapter extends AdapterBase {
  ...
  async _create(data, params = {}) {
    ...
    const select$1 = select(params, this.id);
    ...
    if (isArray) {
      ...
      if (sequelizeOptions.raw) {
        const result2 = instances.map((instance) => {
          if (isPresent(sequelizeOptions.attributes)) {
            return select$1(instance.toJSON());
          }
      ...
      if (isPresent(sequelizeOptions.attributes)) {
        const result2 = instances.map((instance) => {
          const result3 = select$1(instance.toJSON());
  ...
  async _patch(id, data, params = {}) {
      ...
      if (instances) {
        ...
        if (sequelizeOptions.raw) {
          const result2 = instances.map((instance2) => {
            if (hasAttributes) {
              return select$1(instance2.toJSON());
            }
        ...
        if (hasAttributes) {
          const result2 = instances.map((instance2) => {
            const result3 = select$1(instance2.toJSON());
            return Model.build(result3, { isNewRecord: false });
          });
          return result2;
        }
    ...
    if (isPresent(sequelizeOptions.attributes)) {
      const result = select$1(instance.toJSON());
...
export { ERROR, SequelizeAdapter, SequelizeService, dehydrate, errorHandler, hydrate };
